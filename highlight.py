...
# Fixed headers (always highlight if found)
HEADER_PHRASES = [
...
def _unique_path(base_dir: str, base_name: str, ext: str) -> str:
    """Generate unique file path to avoid overwrite"""
    n = 1
    while True:
...
def process_files(pdf_path, excel_path, mode, output_dir=None):
    ...
    ids_set = set(ids_list)  # fast lookup
    ...
    for pno, page in enumerate(doc):
        page_marked = False
        words = page.get_text("words")  # list of words with positions
        ...
        if mode.lower() == "uan":
            ...
            # Full row as single block
            for w in words:
                text = w[4].strip()
                if text in ids_set:
                    found_ids.add(text)
                    y0, y1 = w[1], w[3]
                    # Get all words in same row
        ...
        # --- Add page to output if highlighted ---
        if page_marked:
