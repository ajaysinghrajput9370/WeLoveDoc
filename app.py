...
# Razorpay
RAZORPAY_KEY_ID = os.environ.get("RAZORPAY_KEY_ID")  # use live/test keys
...
def create_order():
    ...
    razorpay_order = razorpay_client.order.create(dict(amount=amount,currency=currency,receipt=receipt,payment_capture='1'))
...
def index():
    ...
    return render_template("index.html", user=user, is_subscribed=is_subscribed, tasks_done=tasks_done)
...
def highlight_route():
    ...
    if "user_id" not in session:
        return "<h3>Please login to use this feature.</h3><a href='/login'>Login</a>",403
    conn = get_db_connection()
    c = conn.cursor()
    ...
    if not is_subscribed and tasks_done>=2:
        return """<h3>Free task limit reached (2/2). Please subscribe.</h3><a href='/plans'>View Plans</a>""",403
    ...
    if not pdf_file or not excel_file or highlight_type not in {"uan","esic"}:
        flash("Please upload both PDF and Excel and select type.","danger")
        return redirect(url_for("index"))
    pdf_path=save_uploaded_file(pdf_file)
    excel_path=save_uploaded_file(excel_file)
    out_pdf, not_found_excel = process_files(pdf_path, excel_path, mode=highlight_type, output_dir=UPLOAD_FOLDER)
    conn = get_db_connection()
    c = conn.cursor()
...
@app.route("/refunds"); def refunds(): return render_template("refunds.html")
@app.route("/shipping"); def shipping(): return render_template("shipping.html")
